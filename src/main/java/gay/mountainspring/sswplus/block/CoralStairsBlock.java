...
public class CoralStairsBlock extends StairsBlock implements Coral {
	public static final MapCodec<CoralStairsBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance.group(CoralBlockBlock.DEAD_FIELD.forGetter(block -> block.deadBlock),
					BlockState.CODEC.fieldOf("base_state").forGetter(block -> block.baseBlockState),
					createSettingsCodec())
			.apply(instance, CoralStairsBlock::new));
	
	protected final Block deadBlock;
	
	@Override
	public MapCodec<CoralStairsBlock> getCodec() {
		return CODEC;
	}
	
	public CoralStairsBlock(Block deadBlock, BlockState baseBlockState, Settings settings) {
